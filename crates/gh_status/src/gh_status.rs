...
impl GhStatusStore {
    ...
    fn refresh_branch(&mut self, key: WatchKey, cx: &mut Context<Self>) {
        ...
        let with_rollup = watched.needs_rollup();
        watched.refresh_task = Some(cx.spawn(async move |this, cx| {
            let executor = cx.background_executor().clone();
            let result = fetch_prs(&key.repo_path, &key.subject, with_rollup, &executor).await;
            this.update(cx, |this, cx| {
                this.finish_refresh(&key, result, with_rollup, cx)
            })
            .ok();
        }));
    }
    ...
    fn finish_refresh(
        ...
        match result {
            Ok(mut prs) => {
                if !with_rollup {
                    carry_over_checks(watched.prs.as_deref().unwrap_or_default(), &mut prs);
                }
            ...
            Err(error) => {
                ...
                if is_rate_limited(&message) {
                    ...
                    if self.rate_limited_until.is_none_or(|prior| prior < until) {
                        log::warn!(
                            "gh_status: GitHub's API budget is spent; not asking again for {} minutes",
                            RATE_LIMIT_BACKOFF.as_secs() / 60
                        );
                    }
...
impl WatchedBranch {
    ...
    /// Whether the next fetch has to carry the check rollup, which is the
    /// expensive half of the query. It only matters while a run could be
    /// moving, so a branch whose checks have settled asks for the cheap
    /// fields and keeps the rollup it already has.
    fn needs_rollup(&self) -> bool {
        self.prs.as_ref().is_none_or(|prs| {
            prs.iter().any(|pr| {
                matches!(pr.state, PrState::Open | PrState::Draft)
                    && matches!(pr.checks, ChecksState::Pending)
            })
        })
    }
}

/// What every fetch asks for. Cheap: one row per PR, whatever its shape.
const GH_JSON_FIELDS: &str =
    "number,url,title,state,isDraft,reviewDecision,mergeable,mergeStateStatus";

/// The check rollup, asked for on top of [`GH_JSON_FIELDS`] only when it could
/// have moved. GraphQL charges by the shape of the query and a rollup is a
/// nested list per PR, so this one field is most of what a poll costs.
const GH_ROLLUP_FIELD: &str = ",statusCheckRollup";
...
/// Keeps the check state a cheap fetch did not ask for. A PR the new list
/// still carries keeps the rollup the last full fetch found; one that is new
/// to the list has none to keep, and the next poll asks for the rollup because
/// its checks read as unknown.
fn carry_over_checks(previous: &[PrStatus], prs: &mut [PrStatus]) {
    for pr in prs {
        let Some(known) = previous.iter().find(|known| known.number == pr.number) else {
            continue;
        };
        pr.checks = known.checks;
        pr.failing_checks = known.failing_checks.clone();
        pr.extra_failing_checks = known.extra_failing_checks;
    }
}
...
async fn fetch_prs(
    ...
    let mut fields = String::from(GH_JSON_FIELDS);
    if with_rollup {
        fields.push_str(GH_ROLLUP_FIELD);
    }
...
mod tests {
    ...
    fn a_branch_is_polled_as_often_as_it_could_change() {
        ...
        // Never fetched: ask now, and ask for the rollup, since nothing is
        // known about what the checks are doing.
        assert_eq!(branch(None).poll_interval(), Some(PENDING_POLL_INTERVAL));
        assert!(branch(None).needs_rollup());
        ...
        assert!(running.needs_rollup());

        // Green and open: what moves it is a push or a review, and a push
        // refreshes it directly. The rollup cannot move, so it is not asked for.
        let green = branch(Some(one(PrState::Open, ChecksState::Passing)));
        assert_eq!(green.poll_interval(), Some(IDLE_POLL_INTERVAL));
        assert!(!green.needs_rollup());
    ...
    /// A poll that did not ask for the rollup keeps the checks it knew. Losing
    /// them would make a green PR read as having no checks every other poll.
    #[test]
    fn a_cheap_poll_keeps_the_checks_it_did_not_ask_about() {
        let mut known = green_pr();
        known.failing_checks = vec!["build".into()];
        known.extra_failing_checks = 2;
        known.checks = ChecksState::Failing;

        let mut fetched = green_pr();
        fetched.checks = ChecksState::None;
        fetched.failing_checks = Vec::new();
        fetched.extra_failing_checks = 0;
        fetched.review = ReviewState::ChangesRequested;

        let mut prs = vec![fetched];
        carry_over_checks(std::slice::from_ref(&known), &mut prs);

        assert_eq!(prs[0].checks, ChecksState::Failing);
        assert_eq!(prs[0].failing_checks, vec![SharedString::from("build")]);
        assert_eq!(prs[0].extra_failing_checks, 2);
        // Everything the cheap fields do cover is the fetch's own answer.
        assert_eq!(prs[0].review, ReviewState::ChangesRequested);

        // A PR the branch had not seen before has nothing to carry over, and
        // its unknown checks are what make the next poll ask for the rollup.
        let mut newcomer = green_pr();
        newcomer.number = 10462;
        newcomer.checks = ChecksState::Pending;
        let mut prs = vec![newcomer];
        carry_over_checks(std::slice::from_ref(&known), &mut prs);
