...
impl DiffMultibuffer {
    ...
    fn buffer_ranges_changed(
        ...
        // A file whose diff appears only now — it had no hunks when the view
        // opened — enters the multibuffer here rather than in `refresh`, and
        // gets its one chance to fold here too. Dropping the answer left a
        // generated file recognised, sorted to the bottom, and open.
        if let Some(buffer_id) = self.register_buffer(
            repo_path,
            path_key,
            file_status,
            display_buffer,
            ...
            self.editor.update(cx, |editor, cx| {
                editor
                    .rhs_editor()
                    .update(cx, |editor, cx| editor.fold_buffers(vec![buffer_id], cx));
            });
        }
    }
