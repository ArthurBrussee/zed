...
impl ProjectDiff {
    ...
    pub fn folded_excerpt_file_paths(&self, cx: &App) -> Vec<String> {
        self.diff.read(cx).folded_excerpt_file_paths(cx)
    }
}
...
mod tests {
    ...
    #[gpui::test]
    async fn test_generated_files_sort_last_and_start_folded(cx: &mut TestAppContext) {
        init_test(cx);

        let generated_header = "// Code generated by a tool. DO NOT EDIT.\n";
        let fs = FakeFs::new(cx.executor());
        fs.insert_tree(
            path!("/project"),
            json!({
                ".git": {},
                "api": { "service.pb.go": "PB\n" },
                "notes.md": format!("{generated_header}NOTES\n"),
                "src": { "main.rs": "MAIN\n" },
            }),
        )
        .await;
        ...
        let folded = diff.read_with(cx, |diff, cx| diff.folded_excerpt_file_paths(cx));
        assert_eq!(folded, vec!["api/service.pb.go", "notes.md"]);
    }

    #[gpui::test]
    async fn test_a_generated_file_appearing_after_the_view_opens_starts_folded(
        cx: &mut TestAppContext,
    ) {
        init_test(cx);
        ...
        let paths = diff.read_with(cx, |diff, cx| diff.excerpt_file_paths(cx));
        assert_eq!(paths, vec!["src/main.rs", "api/service.pb.go"]);
        let folded = diff.read_with(cx, |diff, cx| diff.folded_excerpt_file_paths(cx));
        assert_eq!(folded, vec!["api/service.pb.go"]);
    }
