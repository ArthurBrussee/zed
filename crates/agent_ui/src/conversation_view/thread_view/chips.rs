...
impl ThreadView {
    ...
    /// The devshell a command ran in, when one wrapped the whole line. The
    /// wrapper is stripped from the label (`nix develop … --command ruff check`
    /// is a ruff run), so this is the only thing that still says where it ran.
    pub(super) fn command_environment_for(&self, tool_call: &ToolCall, cx: &App) -> Option<String> {
        if tool_call.terminals().next().is_none() {
            return None;
        }
        self.chip_cache
            .command(tool_call, cx)
            .parsed
            .environment
            .clone()
    }
    ...
    pub(super) fn image_hover_card(
        ...
        Some(chip_hover_card(move |_window, _cx| {
            ...
            // resizes under the pointer is a card that gets away.
            div()
                .w(rems(28.))
                .h(IMAGE_CHIP_HEIGHT)
                ...
                ActionChip::ToolCall { entry_ix } => {
                    ...
                    if is_expanded {
                        ...
                        if let Some(image) = self.tool_call_image(tool_call, cx) {
                            segments.push(self.render_inline_image(entry_ix, image, cx));
            ...
            .when_some(environment, |this, environment| {
                this.child(
                    h_flex()
                        ...
                        .child(
                            Label::new(environment.clone())
                                .size(LabelSize::XSmall)
                                .color(Color::Muted)
                                .buffer_font(cx),
                        )
                        .tooltip(Tooltip::text(format!(
                            "Ran in the {environment} Nix devshell"
                        ))),
                )
            })
            .child(label_element)
    ...
    fn render_inline_image(
        ...
        let body = div()
            .id(("chip-image", entry_ix))
            // A definite height. An image contributes no height until it has
            // loaded, and an entry that grows after the list has measured it
            // paints over the entries below. The picture is fitted inside this
            // box rather than deciding it.
            .h(IMAGE_CHIP_HEIGHT)
            .child(
                picture
                    .size_full()
