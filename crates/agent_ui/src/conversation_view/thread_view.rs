...
#[derive(Clone)]
enum ChipImage {
    /// An image on disk, which may live outside the project.
    File(std::path::PathBuf),
    /// Image data the call carried, with no file behind it.
    Data(Arc<gpui::Image>),
}
...
impl ThreadView {
    ...
    fn thread_pr_chips(&self, cx: &App) -> Vec<ui::ThreadItemPrChip> {
        ...
        gh_status::pr_chips_for_branches(
            branches
                .iter()
    ...
    /// The Stop button, pinned to the input box's top-right corner. It is
    /// never gated on the input's contents, so cancelling a turn is always
    /// one click.
...
impl ThreadView {
    ...
    fn tool_call_image(&self, tool_call: &ToolCall, cx: &App) -> Option<ChipImage> {
        if let Some((image, _)) = tool_call.content.iter().find_map(|content| content.image()) {
            return Some(ChipImage::Data(image.clone()));
        }
    ...
    fn render_image_output(
        ...
        v_flex()
            ...
            .child(
                // Same definite box as an inline image chip: the picture is
                // fitted into it rather than deciding the entry's height once
                // it loads.
                div()
                    .h(IMAGE_CHIP_HEIGHT)
                    .max_w_96()
