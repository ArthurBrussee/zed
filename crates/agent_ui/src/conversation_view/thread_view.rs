...
pub struct ThreadView {
    ...
    /// How many entries the PR miner has already read. Entries are appended,
    /// so each is scanned once; the last is re-read while it is still being
    /// written to, which is what catches a URL as a command prints it.
    mined_entries: usize,
...
fn finished_command_id(entry: &AgentThreadEntry, cx: &App) -> Option<acp::ToolCallId> {
...
impl ThreadView {
    ...
    pub(crate) fn mine_pr_mentions(&mut self, cx: &mut Context<Self>) {
        ...
        // A pass that starts at the first entry has read the whole thread, so
    ...
    pub(crate) fn dismiss_pr(&mut self, pr: WatchedPr, cx: &mut Context<Self>) {
        ...
        let thread_id = self.root_thread_id;
        store.update(cx, |store, cx| {
            store.update_pr_snapshot(thread_id, |snapshot| snapshot.dismiss(&pr), cx);
        });
        cx.notify();
    }
    ...
    pub(crate) fn watch_pr(&mut self, pr: WatchedPr, cx: &mut Context<Self>) {
        ...
        let thread_id = self.root_thread_id;
        store.update(cx, |store, cx| {
            store.update_pr_snapshot(thread_id, |snapshot| snapshot.add(pr), cx);
        });
        cx.notify();
    }
    ...
    fn build_add_pr_menu(
        ...
        ContextMenu::build(window, cx, move |mut menu, _, _| {
            ...
            menu = menu.separator().header("Seen Before");
    ...
    fn pr_menu_candidates(&self, already: &[WatchedPr], cx: &App) -> Vec<PrMenuCandidate> {
        let Some(store) = ThreadMetadataStore::try_global(cx) else {
            return Vec::new();
        };
...
mod tests {
    ...
    fn test_tool_call(
        ...
        let mut call = acp::ToolCall::new(id.to_string(), title.to_string()).kind(kind);
    ...
    fn a_command_that_has_printed_nothing_has_no_line_for_the_band(
        ...
        cx.update(|cx| {
            ...
            tool_call.set_content_for_test(vec![acp_thread::ToolCallContent::Terminal(terminal)]);
