...
use crate::{
    ...
    conversation_view::{
        AcpThreadViewEvent, DraftWorktreeChoice, RootThreadUpdated, ThreadView,
...
pub struct AgentPanel {
    ...
    /// The draft whose message should auto-submit in the destination workspace
    /// once a send-triggered worktree switch carries it over. Set while
    /// [`AgentPanel::create_worktree_and_send`] is creating the worktree, so
    /// the destination pull knows this switch came from a send.
...
impl AgentPanel {
    ...
    /// Creates (or focuses) an in-place draft and records that sending it
    /// should create a new git worktree off `branch_target`. Nothing is
    /// created up front: the worktree, its workspace, and the thread are all
    /// created only when the user sends (see [`Self::create_worktree_and_send`]).
    ...
    /// workspace's panel opens with a fresh thread. Unlike a new-worktree
    /// draft, nothing is deferred: the worktree exists as soon as this
    /// returns.
    ...
    pub fn new_worktree_draft(
        ...
        // Record the choice on the ConversationView, which exists now even
        // though its thread view connects asynchronously.
        if let Some(draft) = self.draft_thread.clone() {
            draft.update(cx, |draft, cx| {
                draft
                    .set_draft_worktree_choice(DraftWorktreeChoice::NewWorktree(branch_target), cx);
            });
        }
        cx.notify();
    }

    /// Creates the git worktree and its workspace on first send, then starts
    /// the thread in the new workspace with `blocks` as its first message,
    /// which auto-submits once the session connects. Delivery is direct: the
    /// created workspace hands back its own handle, so the message never rides
    /// the workspace-switch pull (whose ordering against this task is not
    /// guaranteed, which used to land an empty thread in the new worktree).
    /// Nothing is created before this send, so there is no dummy thread.
    /// Creation failure restores the message to the source draft and toasts.
    pub fn create_worktree_and_send(
        &mut self,
        ...
        window
            .spawn(cx, async move |cx| {
                let result = async {
                    let created = task.await.context("failed to create worktree workspace")?;
                    // The panel is registered by the time creation returns:
                    // `open_worktree_workspace` awaits the panels task and the
                    // initial scan.
                    let target_panel = created
                        .workspace
                        .read_with(cx, |workspace, cx| workspace.panel::<AgentPanel>(cx))
                        .context("new workspace did not register an agent panel")?;
                    target_panel.update_in(cx, |panel, window, cx| {
                        // The fresh panel auto-created an empty draft tab at
                        // load; the delivered thread replaces it rather than
                        // sitting beside a stray "New Thread" tab. Typed
                        // content (a fast user) is never discarded.
                        if panel
                            .draft_thread
                            .clone()
                            .is_some_and(|draft| !panel.draft_has_content(&draft, cx))
                        {
                    ...
                    // The message now lives in the new worktree's thread, so
                    // drop the source draft's copy. Only on success: a failed
                    // creation must leave the draft usable in place.
                    source_workspace
                        .update_in(cx, |workspace, window, cx| {
                            if let Some(panel) = workspace.panel::<AgentPanel>(cx) {
                                panel.update(cx, |panel, cx| {
                                    panel.finish_worktree_send(source_thread_id, true, window, cx);
                                });
                            }
                        })
                        .ok();
                ...
                if let Err(error) = result {
                    log::error!("Failed to start a thread in a new worktree: {error:#}");
                    source_workspace
                        .update_in(cx, |workspace, window, cx| {
                            if let Some(panel) = workspace.panel::<AgentPanel>(cx) {
                                panel.update(cx, |panel, cx| {
                                    panel.finish_worktree_send(source_thread_id, false, window, cx);
                                });
                            }
                            struct CreateWorktreeThreadError;
                            workspace.show_toast(
                                workspace::Toast::new(
                                    workspace::notifications::NotificationId::unique::<
                                        CreateWorktreeThreadError,
                                    >(),
    ...
    /// Routes a resolved new-worktree send to its draft, which clears the
    /// in-flight guard and, on success, drops the composed message and resets
    /// the worktree choice.
    fn finish_worktree_send(
        &mut self,
        thread_id: ThreadId,
        success: bool,
        window: &mut Window,
        ...
        if let Some(conversation_view) = self.conversation_view_for_id(&thread_id, cx) {
            conversation_view.update(cx, |conversation_view, cx| {
                conversation_view.finish_worktree_send(success, window, cx);
            });
        }
        // The message lives in the new worktree now; the source draft tab has
        // nothing left to say. Keep it only when it is the pane's last thread
        // tab (every workspace keeps one).
        if success {
            let has_other_tabs = self
                .thread_pane
                .read(cx)
                .items_of_type::<crate::thread_tab::ThreadTab>()
                .any(|tab| tab.read(cx).thread_id(cx) != thread_id);
            if has_other_tabs {
                if self
                    .draft_thread
                    .as_ref()
                    .is_some_and(|draft| draft.read(cx).thread_id == thread_id)
                {
                    ...
                    self.close_thread_tab(thread_id, window, cx);
                }
            }
        }
    }
    ...
    /// Sets the active draft's worktree target. Used by the sidebar's
    /// per-worktree new-thread button to pin the draft to that worktree.
    pub fn set_active_draft_worktree_choice(
        &mut self,
        choice: crate::DraftWorktreeChoice,
        cx: &mut Context<Self>,
    ) {
        if let Some(draft) = self.draft_thread.clone() {
            draft.update(cx, |draft, cx| {
                draft.set_draft_worktree_choice(choice, cx);
            });
        }
    }
...
mod tests {
    ...
    async fn test_enter_sends_an_unstarted_draft(cx: &mut TestAppContext) {
        ...
        // Drafts default to a new worktree; this test drives the
    ...
    async fn test_thread_without_an_agent_title_generates_one_locally(cx: &mut TestAppContext) {
        ...
        fake_model.send_last_completion_stream_text_chunk("Sidebar PR badges");
    ...
    async fn test_agent_supplied_title_is_not_regenerated(cx: &mut TestAppContext) {
        ...
        assert!(
            model.as_fake().pending_completions().is_empty(),
    ...
    async fn test_worktree_switch_carries_source_draft_into_destination(cx: &mut TestAppContext) {
        ...
        // What `create_worktree_and_send` does once the content moved over.
        panel_a.update_in(cx, |panel, window, cx| {
