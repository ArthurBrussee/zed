...
impl AgentPanel {
    ...
    fn new_thread_pane(
        ...
        let pane = cx.new(|cx| {
            ...
            // A thread tab draws its own running indicator inside its title and
            // never a close button, so the slots an editor tab reserves for
            // those are width it can give back to the title.
            pane.set_tabs_fit_content(true, cx);
            pane
        });
