            ...
            Some(dragged) => {
                ...
                div()
                    ...
                    .drag_over::<DraggedThreadRow>(move |style, dragged, _, cx| {
                        // A row only offers to take a drop it would accept:
                        // another worktree's row refuses it, and so does the
                        // dragged row itself. Both used to light up and then
                        // swallow the drop, which reads as the drag breaking.
                        if dragged.workspace.entity_id() != group
                            || dragged.thread_id == thread_id
                        {
                            return style;
                        }
                        let color = cx.theme().colors();
