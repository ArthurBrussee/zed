...
impl ToolCall {
    ...
    fn update_fields(
        ...
        let was_plain_text = self.title.is_none();
        // An edit's label is derived from the files it touches, so new
        // locations change it just as a new title does.
        let mut label_changed = title.is_some() || kind.is_some() || locations.is_some();
        ...
        // The locations of this same update win over the ones already on the
        // call: an edit's label is derived from the files it touches.
        let label_locations = locations.clone().unwrap_or_else(|| self.locations.clone());
        if label_changed {
            let is_plain_text = self.title.is_none();
...
impl RunningWork {
    pub fn is_empty(&self) -> bool {
        self.terminals == 0 && self.subagents == 0
    }
}
...
mod tests {
    ...
    async fn test_running_work_counts_only_what_is_still_in_flight(cx: &mut gpui::TestAppContext) {
        ...
        let terminal_id = acp::TerminalId::new("running-command");
        thread.update(cx, |thread, cx| {
            let builder = TerminalBuilder::new_display_only(
                CursorShape::default(),
                AlternateScroll::On,
            ...
            thread
                .handle_session_update(
                    acp::SessionUpdate::ToolCall(
                        acp::ToolCall::new("terminal-call", "cargo test")
            ...
            thread
                .handle_session_update(
                    acp::SessionUpdate::ToolCall(
                        acp::ToolCall::new("subagent-call", "Investigate the failure")
                            .status(acp::ToolCallStatus::InProgress)
            ...
            thread
                .handle_session_update(
                    acp::SessionUpdate::ToolCallUpdate(acp::ToolCallUpdate::new(
                        acp::ToolCallId::new("subagent-call"),
    ...
    async fn test_terminal_output_records_how_long_the_command_ran(cx: &mut gpui::TestAppContext) {
        ...
        thread.update(cx, |thread, cx| {
            thread.on_terminal_provider_event(
                TerminalProviderEvent::Exit {
                    terminal_id: terminal_id.clone(),
                    status: acp::TerminalExitStatus::new().exit_code(Some(0)),
                },
                cx,
            );
        });
