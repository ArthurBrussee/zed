...
pub struct ParsedCommand {
    ...
    /// The devshell the whole line ran in, when every working segment used the
    /// same one.
...
pub fn parse_command(command: &str) -> ParsedCommand {
    ...
    // The line as a whole ran remotely only when every working segment did,
    // on the same machine. The devshell reads the same way: one badge for the
    // line, or none.
    let common = |value: fn(&CommandSegment) -> Option<String>| {
        let mut values = segments
            .iter()
            .filter(|segment| !segment.kind.is_noop())
            .map(value);
        match values.next() {
            Some(Some(first)) if values.all(|other| other.as_deref() == Some(first.as_str())) => {
                Some(first)
            }
            _ => None,
        }
    ...
    let host = common(|segment| segment.host.clone());
    let environment = common(|segment| segment.environment.clone());
...
fn classify_segment(segment: &str) -> SegmentKind {
    ...
    for (index, stage) in split_stages(segment).into_iter().enumerate() {
        ...
        if matches!(kind, SegmentKind::Search { .. })
            && matches!(result, SegmentKind::Noop | SegmentKind::Read { .. })
        {
            return kind;
        }
