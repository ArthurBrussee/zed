...
impl SegmentKind {
    ...
    /// Whether this segment says something about the project rather than
    /// computing a value: looking through it, changing it, asking git about
    /// it. This is what a line keeps when all it did with a command was
    /// capture its output — `f=$(grep -rln sym src)` searched the tree, while
    /// `t=$(date +%s)` produced a number and touched nothing.
...
/// Programs that only report state — the machine's, or a file's. They change
/// nothing and their whole output exists to be read, so a filter over one is
/// reading the state rather than trimming the output of work.
///
/// The last four read a file (or a path) and print what they found; none of
/// them can write, which is why `jq` is here and `yq` is not (`yq -i` edits in
/// place).
...
/// The `gh` subcommands that only ask GitHub a question. Viewing and listing
/// pull requests, runs and issues is looking around in exactly the way `git
/// status` is, and a run of chips should not break for one.
///
/// The verb is the second word (`gh pr view`, `gh run list`), and anything not
/// listed reads as work. That default is the safe one: an unrecognised
/// look-around only costs an extra chip, while an unrecognised action would
/// hide one. `api` is deliberately absent — `gh api` is a GET until someone
/// passes `-X POST`.
const GITHUB_READ_ONLY_ACTIONS: &[&str] = &["view", "list", "status", "checks", "diff", "watch"];

/// Whether a `gh` invocation only asked GitHub about something, given the
/// `operation` string the parser built (`"pr view"`, `"run list"`, or a bare
/// `"pr"` when nothing followed).
...
pub enum CommandClass {
    ...
    GitInfo,
    /// Asking GitHub about state: `gh pr view`, `gh run list`.
    GitHub,
    /// Looking at something that is neither a file, a search, a diff, nor a
    /// repository: `ps`, `df`, `jq .version package.json`.
    Inspect,
    Other,
}
...
pub fn parse_command(command: &str) -> ParsedCommand {
    let segments: Vec<CommandSegment> = split_segments(command)
        .into_iter()
        .flat_map(|text| {
            // A variable capturing a command's output is bookkeeping, but the
            // command inside it is often the only thing on the line that says
            // what the line was looking for: `f=$(grep -rln sym src | head -1)`
            // searched the tree, and the rest of the line only handles what it
            // found. The substitution's own segments stand in for the capture,
            // keeping the ones that touched the project.
...
pub fn classify_command(command: &str) -> CommandClass {
    ...
    for segment in &parsed.segments {
        match &segment.kind {
            ...
            // Asking GitHub about a pull request is looking around the same way
            // `git status` is; opening or merging one is not.
            ...
            // The parser already knows which programs only report state
            // (`is_just_looking`); this used to throw that away and end the run
            // on every `ps` and `jq`.
            SegmentKind::Run { .. } => {
    ...
    // then reading files, then asking a forge how things stand, then merely
...
fn split_segments(command: &str) -> Vec<String> {
    ...
    // How deep inside `$( … )` the scan is, so a captured command line is kept
    // whole.
    ...
    while let Some((index, ch)) = chars.next() {
        ...
        // A command substitution holds a command line of its own, and the
        // `;`, `&&` and newlines inside it belong to that line rather than to
        // this one.
...
/// The command a segment captures into a variable, when that is all the
/// segment is: `f=$(grep -rln sym src | head -1)` captures a search.
...
/// The command inside a `$( … )`, when a word is nothing but one substitution.
fn command_substitution(word: &str) -> Option<&str> {
    let word = strip_matching_quotes(word.trim());
    let inner = word.strip_prefix("$(")?;
    // `$((` opens arithmetic, which computes rather than runs.
    ...
    for (index, ch) in inner.char_indices() {
        ...
        match ch {
            ...
            ')' => {
                depth -= 1;
                // Anything after the substitution closes is part of a larger
                // word (`$(git rev-parse HEAD):path`), which the substitution
                // does not stand for.
                if depth == 0 {
...
/// Whether a word is a variable the shell expanded before the command ran.
/// The line records `$f`; what it stood for is not in the text, so it names
/// no file and no line number.
...
fn classify_stage(stage: &str) -> SegmentKind {
    ...
    while let Some(word) = words.first() {
        ...
        // `code=$?` next to it. What the captured command touched is lifted
        // out of the substitution before a segment is classified, so what
        // reaches here captured a value and nothing else.
    ...
    let paths = |rest: &[&str]| -> Vec<String> {
        rest.iter()
            ...
            .map(|word| strip_matching_quotes(word).to_string())
            // A variable the shell expanded is not a filename: `sed -n … "$f"`
            // read whatever the line found earlier, and the text does not say
            // what that was.
...
mod tests {
    ...
    fn a_captured_command_is_what_the_line_was_looking_for() {
        let command = "f=$(grep -rln \"export function withWristLnsCalibrationCollisionTarget\" \
            portico/src --include='*.ts' | head -1)\n\
            n=$(grep -n \"export function withWristLnsCalibrationCollisionTarget\" \"$f\" | cut -d: -f1)\n\
            echo \"$f\"\n\
            sed -n \"$((n-6)),$((n+25))p\" \"$f\"";
        // The greps are the only statements that say what the line was looking
        // for, and both of them used to be dropped with their assignment.
        assert_eq!(
            short_labels(command),
            [
                "export function withWristLnsCalibrationCollisionTarget",
                "export function withWristLnsCalibrationCollisionTarget",
                "read",
            ],
            "searched for the symbol, then read the file it found"
        );
        // A variable the shell expanded names no file, and an arithmetic
        // range no lines: neither is in the text to be shown.
        assert!(
            matches!(
                kinds(command).as_slice(),
                [
    ...
    #[test]
    fn a_capture_that_computed_a_value_is_still_bookkeeping() {
        // Nothing here touched the project: the substitution produced a
        // number for the lines below it.
        assert_eq!(kinds("t=$(date +%s)"), [SegmentKind::Noop]);
        assert_eq!(kinds("deadline=$(( $(date +%s) + 14400 ))"), [SegmentKind::Noop]);
        // A captured command line stays whole: the `&&` inside the
        // substitution belongs to it, not to the line around it.
        assert_eq!(
    ...
    /// Asking GitHub about a pull request is the agent looking before it acts,
    /// exactly like `git status`. Every one of these used to classify as
    /// `Other`, which ended the run of chips and started another.
    #[test]
    fn asking_github_is_looking_around() {
        assert_eq!(classify_command("gh pr view 123"), CommandClass::GitHub);
        assert_eq!(classify_command("gh pr list"), CommandClass::GitHub);
        assert_eq!(classify_command("gh pr checks"), CommandClass::GitHub);
        assert_eq!(classify_command("gh run list"), CommandClass::GitHub);
        assert_eq!(classify_command("gh run watch 42"), CommandClass::GitHub);
        assert_eq!(classify_command("gh issue view 7"), CommandClass::GitHub);
        assert_eq!(classify_command("gh pr diff"), CommandClass::GitHub);

        // Doing something to a pull request is not looking at one.
        assert_eq!(classify_command("gh pr create"), CommandClass::Other);
        assert_eq!(classify_command("gh pr merge 123"), CommandClass::Other);
        assert_eq!(classify_command("gh pr comment 1 -b x"), CommandClass::Other);
        assert_eq!(classify_command("gh pr checkout 5"), CommandClass::Other);
        // Unrecognised, so it reads as work: an extra chip is the cheap
        // mistake, a hidden action is the expensive one.
        assert_eq!(classify_command("gh api /repos/x/y"), CommandClass::Other);
        assert_eq!(classify_command("gh pr"), CommandClass::Other);

        // A read mixed into a real run of looking still folds with it.
        assert_eq!(
            classify_command("gh pr view 1 && git status"),
            CommandClass::GitInfo
        );
    }

    /// The parser already knew which programs only report state
    /// (`INSPECTION_PROGRAMS`); the classifier threw that away and ended the
    /// run on every one of them.
    #[test]
    fn programs_that_only_report_state_are_looking_around() {
        assert_eq!(classify_command("ps -ax"), CommandClass::Inspect);
        assert_eq!(classify_command("df -h"), CommandClass::Inspect);
        assert_eq!(classify_command("uname -a"), CommandClass::Inspect);
        assert_eq!(
            classify_command("jq .version package.json"),
            CommandClass::Inspect
        );
        assert_eq!(classify_command("basename /a/b.rs"), CommandClass::Inspect);

        // Still work: these run something.
        assert_eq!(classify_command("cargo test"), CommandClass::Other);
        assert_eq!(classify_command("pytest -q"), CommandClass::Other);
        // `yq -i` edits in place, so `yq` is deliberately not an inspection
        // program.
        assert_eq!(classify_command("yq .a f.yaml"), CommandClass::Other);

        // A search over one of these is the point of the line, the same way
        // `cat x | grep y` is; a build filtered by a grep still reads as work.
        assert_eq!(classify_command("ps -ax | rg cargo"), CommandClass::Search);
        assert_eq!(
            classify_command("cargo check | grep '^error'"),
            CommandClass::Other
        );

        // And a run mixing these with file reads is one look-around.
        assert_eq!(
            classify_command("cat Cargo.toml && jq .name package.json"),
            CommandClass::Read
        );
    }
