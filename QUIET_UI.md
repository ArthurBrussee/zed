...
- running: `status() == Generating`. `ui::agent_running_indicator()` (rotating accent spinner)
  renders in sidebar rows and as the thread view's generating indicator. A thread tab draws no
  glyph for it: its own agent icon pulses instead (`render_tab_icon` in `thread_tab.rs`), so a
  tab bar of running threads carries no row of spinners. The dots below are unchanged.
...
**2026-09-13**: onto main 7960b2a7c (58 upstream commits; the branch had not been rebased since
2026-09-08, four runs having not happened). Squash-then-rebase folded the standing squash and
fourteen follow-ups — three code commits from the 09-08 run's own tail and five of Arthur's queue
edits among them — into one commit, reusing the squash's own message; tree-identical to the old
tip (`2cb62d993`) before rebasing.

**Two conflicts, both trivial, and no markerless drift.** `acp_thread.rs` was an import collision
(upstream added `settings::{Settings, SettingsStore}` where the fork had added `std::borrow::Cow`;
kept both). `Cargo.lock` was upstream's `futures 0.3.34` bump (#64077, ACP 2.1) against the fork's
own dependency lines for `git` and `gh_status`; took upstream's version with the fork's additions.
`cargo check --workspace --all-targets` then came back with 0 errors and 0 warnings, which is the
first time a 58-commit replay has cost nothing.

**Upstream now does what one Work queue item asked for.** `util::increase_open_file_limit`
(#64034, landed 2026-09-11, the day after the EMFILE hunt) raises `RLIMIT_NOFILE`'s soft limit to
the hard limit at startup, before anything opens a file, and logs the value it settled on —
called from `main.rs` right after logging is up. There was no fork code to delete, because the
fork had not built it yet; the item's first half was simply already done, and only its second
half (saying why a spawn failed) was built tonight. Checked the rest of the batch for the same
pattern: #63138's markdown highlight caching is inside `MarkdownElement`'s parse task and does not
serve the fork's `highlight_code_runs`, which highlights a bare command string into `TextRun`s;
#63660's terminal-thread renaming is the agent panel's, not the sidebar's; #64107's new
`Window::on_visibility_changed` is a better shape than `observe_window_activation` for one thing
built tonight, but not a duplicate of anything the fork owns.

**Built tonight, in queue order.**

- *Say what a running thread is running.* `AcpThread::running_work` counts the terminals whose
  process has not exited and the subagent calls still `InProgress`, from the entries the thread
  already holds, and the sidebar's live-info walk carries both numbers into the row. They sit on
  the metadata line beside the diff stats rather than beside the spinner in the title row: that
  line is drawn on every row already, so a thread starting a command cannot resize its row, which
  is what the entry asked for even where it named the other slot. The entry's companion — making
  a working row obvious at a glance — turned out to be built already (the accent wash and leading
  edge in `ThreadItem`), so only the counts were missing.
- *A command's work can hide inside `$(...)`.* The capture is still bookkeeping; the command
  inside it is not. `parse_command` now lifts a `VAR=$( … )`'s own segments into the line, keeping
  the ones that touched the project — looking through it, changing it, asking git about it — so
  the complaint's two greps become the line's two searches and `t=$(date +%s)` stays the Noop the
  entry said it should be. `split_segments` also stopped splitting inside a substitution, so a
  captured `cd x && rg y` survives whole. The second half went in too: a variable the shell
  expanded names no file, so `sed -n "$((n-6)),$((n+25))p" "$f"` reports neither a path of `$f`
  nor a line range (the arithmetic range was already rejected by `sed_line_range`). The
  complaint's exact four lines are the test.
- *The command-changed card shows that command's diff.* The hover card drew
  `open_uncommitted_diff`, which answers how the file differs from HEAD. It now builds
  `Diff::finalized` from what the command found against what is on disk now. A file clean at
  command start has HEAD as its before-text and costs nothing; one already dirty has its text
  read as the command starts — `Terminal::watch_repository` already had the baseline status, which
  is exactly that set — capped at 32 files and 2MB each so a wall of dirty files cannot make a
  command expensive to start. A file whose read did not land keeps the wider diff and keeps saying
  so. Nothing persists, and clicking still opens the file's ordinary diff view, which was already
  what the entry wanted. The card's cache is keyed by entry as well as path now: two commands that
  touched one file were sharing a diff.
- *The diff stats refresh on more than the events they watch.* Three more reasons to recompute: a
  command that has finished (its `EntryUpdated` carries a tool call with an exited terminal), the
  window coming back to the front, and a second look 1.5s after a head change, since the 250ms
  debounce can fire halfway through a checkout with nothing left to correct it. The activation
  trigger costs only the thread being looked at: it marks the readout stale and the recompute
  happens when that thread next draws it.
- *A running command's output line moved under the chip.* It was a fixed nine-rem box in the
  chip's own row, competing with the label. It now sits in a band the chip reserves beneath the
  label — absolutely positioned, so out of the row's flow — with the chip's full width and an
  ellipsis at the end. The band is there for as long as the command runs rather than arriving with
  the first line of output, which is the constraint the entry named; the chip gives the band back
  when the command ends and tells `ListState` to remeasure that entry, which is the one height
  change this introduces.
- *A spawn that fails says why.* Half of this item was upstream's already (above). The other half:
  `util::process::Child::spawn` wrapped the OS error as context, so the reason printed only under
  `{:#}`, and the two places that turn a launch failure into UI (`ConversationView`'s load path
  and `AgentConnectionStore`) both used `to_string()`. The reason is in the message now, `EMFILE`
  and `ENFILE` are named as running out of file handles rather than left as an error number, and
  both paths log the full chain — the agent launch failure reached the UI without reaching the log
  at all before this.

**Not built, and why.** Three items are left in the queue and none of them was skipped for size.
The thread-stuck-in-loading item and the two below it are the tail the clock reached: the gate
needs its hours, and starting a launch-path audit at 22:30 would have handed back half of one. The
spare worktree stays measurement-gated for the reason the 2026-09-07 and 09-08 runs recorded, and
the two per-rebuild costs want the `quiet-ui perf: sidebar rebuilt` line's numbers from the running
app before a store change is worth making — this sandbox has no macOS and no display, so it cannot
produce either.

**Tested, and what could not be.** The parser change has the complaint's exact four lines as a
test, plus one for the capture that only computed a value and one for a captured `&&` staying
whole. `AcpThread::running_work` has a test that counts a live terminal and a live subagent and
then watches both leave. The pre-command capture has one that reads a dirty file's text and keeps
nothing for a clean one. `util::process` has two: a spawn failure that names its reason, and
`EMFILE`/`ENFILE` being told from `ENOENT`. What has no test is the diff-stats refresh triggers:
they are a window activation and a finished command's terminal, and asserting that a git diff was
recomputed for one of those reasons rather than because the repository's own watcher noticed
would be a test asserting nothing. The hover card's diff has none either, for the same reason the
declared-edit cards have none — it is an editor built in a tooltip.

**One upstream test failed, and it was right to.** #63660 brought
`test_rename_selected_thread_action_does_not_edit_terminal_from_agent_thread`, which asserts that
a rename dispatched while an agent thread is showing does not open a terminal's title editor. It
failed here, and not only on its surface assumption (in this fork a thread is a tab, never the
panel's base view). The behaviour underneath was wrong too: a thread tab activated while a
terminal held the base view left the terminal on the panel, so `active_terminal_id()` still
answered with it and the rename would have edited the terminal's title while the user was looking
at a thread. `activate_thread_tab` and `open_thread_tab` now give the terminal base view up — but
only for a focused activation, since restore opens tabs behind whatever surface the user left
showing, which is what `test_active_terminal_serialize_and_load_round_trip` is there to keep. The
upstream test was adapted to activate the draft the way the New Thread action does and to assert
the panel has left the terminal rather than that a thread is the base view.

- *A thread stuck in "loading" now says so.* The entry asked for the fix that does not depend on
  finding the trigger, and the trigger turned out to be findable anyway. `AgentConnectionStore`
  keeps one entry per agent and hands it to every thread that asks; an entry stuck in `Connecting`
  is therefore the guard the entry's own `gh_status` analogy describes — one wedged launch is every
  later thread waiting on it, silently, with `restart_connection` joining it too rather than
  retrying. The launch has a three-minute deadline now, each step it passes is logged with its
  elapsed time (connecting, each loading status the agent server reports, the spawn and its pid,
  connected), and past the deadline the thread gets an error it can see and retry while the entry
  is dropped so the next thread launches instead of joining. Tested with a server whose `connect`
  never returns: the clock advances, the status goes to disconnected, the entry is gone.
  Of the three places the entry told the next run to check, the npm-install gate is sound (it is
  cleared by whoever finishes waiting, which the 2026-09-07 run already got right) and the
  project's shell environment was not: it is cached by (shell, directory) for the life of the
  process, with no timeout under it, so a login shell that never returns held every later launch in
  that directory with nothing in the log. It gives up after 45 seconds now and says so.

**The gate.** Suites: the fork's core crates plus every crate touched tonight — `acp_thread` (234),
`sidebar` (175), `agent_ui` (455, 32 intentionally `#[ignore]`d), `agent_servers` (33), `ui` (41
doctests), `util` (108 + 2 doctests), `project` (62 lib + 397 integration, 4 ignored). The Verification queue was empty
going in and is empty going out. `script/clippy -p util -p ui -p gpui_macros -p acp_thread -p sidebar
-p agent_ui -p agent_servers -p project` (release, all targets, all features) is clean, after one
fix it caught that the suites cannot: a `get(&key).is_none()` in tonight's new connection-store
test.

**A third note, new: `-p ui` cannot be linted on its own in release.** `script/clippy` passes
`--all-features`, which applies only to the packages selected, and `ui`'s
`derive_inspector_reflection` feature does not forward to `gpui_macros`'s `inspector` one. With
`debug_assertions` off, `ui` then asks for a macro that was configured out and the release build
fails before any lint runs — nothing to do with the fork. Adding `-p gpui_macros` to the same
invocation selects the package whose feature the macro lives behind, and the whole union lints
clean.

**Two gate-invocation notes, both of them old friends.** `cargo test -p <crate>` piped through
`tail -6` can swallow the lib result line entirely: the sidebar's run looked like "0 passed" when
it had in fact passed 175, because only the doctest result survived the tail. And the disk
allowance behaved exactly as 2026-09-07 and 09-08 recorded: the debug test tree filled it partway
through `project` (`No space left on device` while linking), and `rm -rf target/debug` between
passes fit comfortably. Budget one clean between the test pass and the release clippy, and one more
before a large crate's test build.
...
**2026-09-16**: onto main 8eebe1ce1 (32 upstream commits). Squash-then-rebase folded the standing
squash and the three follow-ups from the 09-15 run — its sidebar-`Arc` commit, its rebase-log
commit, and its dead-helper deletion — into one, reusing the squash's own message; tree-identical
to the old tip (`f3663631c`) before rebasing. A working night by the more-than-one-commit trigger
alone: both queues were empty going in, and the branch had moved yesterday.

**One file conflicted, and no markerless drift.** `cargo check --workspace --all-targets` came back
0 errors and 0 warnings on the first try — the second night running that the conflict count was the
whole story rather than the usual half of it.

The conflict is `crates/editor/src/element.rs`, and it is an import list: two adjacent lines, each
side having changed one. Upstream added `BTreeSet` to the `collections` import; the fork had deleted
the line below it, `feature_flags::{DiffReviewFeatureFlag, FeatureFlagAppExt as _}`, because it
un-flags diff review so the affordance is always live. Resolved the standing way — the fork's
behaviour, upstream's shape: `BTreeSet` in, the `feature_flags` line stays out. Checked afterwards
that no `DiffReviewFeatureFlag` or `has_flag` reference survives anywhere in the file, and that
`BTreeSet` is genuinely used, since an import-list conflict is precisely where a stranded symbol
hides and neither the merge nor the compiler would have said so.

**Nothing for the fork to delete tonight, but the survey found two things worth keeping.** Thirteen
of the fork's 90 changed files overlap with what upstream touched in these 32 commits; twelve of
them auto-merged, which is where a silent loss would live, so they were read rather than trusted.

- #64259 adds `agent.threads_sidebar_auto_open`, gating the `open_sidebar` call in `open_paths` so
  opening a folder in an existing window no longer force-opens the threads sidebar. It merged into
  `workspace.rs` untouched and the fork consumes it for free. An upstream feature the fork did not
  have, not a duplicate of one it did.
- #64195's Copilot token-limit rework rewrites the token-usage block in `thread_view.rs`, the
  fork's most-rewritten file. It landed intact — `max_input_tokens` and `input_token_capacity()`
  sit at the right places in the fork's copy. Recorded because this is the file where a silent
  loss would be hardest to notice.
- **Checked and not redundant, recorded here for the first time: upstream's `shell_command_parser`
  crate.** It has never appeared in this log, and it is the obvious thing for a future run to
  mistake for a replacement for `command_parse.rs`. It is not one. It is a thin wrapper over
  `brush_parser`'s bash AST exposing three functions — `extract_commands`,
  `extract_terminal_command_prefix`, `validate_terminal_command` — and it exists to normalise a
  command prefix for tool-permission matching ("always allow `git status`"). It hands back command
  strings and a normalised prefix; it does not expose per-segment argument positions, which is the
  entire input to the chip layer's line ranges, revisions, and read/write/destructive
  classification. It predates the fork's parser and is not a thing upstream built since. Nothing to
  adopt; a run that rediscovers it should stop at this paragraph.
- Upstream still has no equivalent of `generated_file.rs`, `worktree_spares.rs`,
  `thread_read_state.rs`, or the `gh_status` crate, checked by name and by concept.
- #64260 strips `rodio`'s default features but keeps `playback`, so `libasound2-dev` remains a
  prerequisite here. Worth knowing before someone reads the commit title and drops it.

**Nothing was built: the Work queue was empty going in, and stays empty.** The Verification queue
was empty too. The night went to the rebase, the survey above, and the widest gate this log has
recorded.

**The dead-`pub` sweep, run and empty.** 58 `pub fn`s across the fork's thirteen own modules, every
one with a caller. Expected, since no fork code has landed since the 09-15 sweep found `send_draft`,
but it is grep-cheap and the 09-08 entry's point stands: a `pub` item with no callers raises no
warning and is only ever found by looking.

**The gate, widened to every crate the fork touches — twenty of them, not three.** With no queue to
build, the runway went here instead, the way 2026-09-01 did it. `cargo test`: `acp_thread` 241,
`agent_ui` 460 (32 intentionally `#[ignore]`d), `sidebar` 180, `editor` 1134, `agent_servers` 34,
`auto_update` 15, `fs` 24, `gh_status` 28, `git_ui` 160, `git_ui_core` 31, `markdown` 158,
`picker` 7, `theme_settings` 8, `title_bar` 7, `ui` 82 (+41 doctests), `util` 108 (+2 doctests),
`zed_actions` 0, `workspace` 276, `project` 398 (3 `#[ignore]`d), `gpui_macros` 3.
`script/clippy` (`--release --all-targets --all-features -- --deny warnings`) is clean across all
twenty, in two invocations. Counts for the three core crates are identical to 09-15, so nothing
tonight moved them.

**Three failures, every one established as upstream's rather than assumed to be.**

- *`editor`'s `test_code_lens_resolve_only_visible`* is the one 2026-09-01 already chased down, and
  it has not changed: `{60, 70, 80, 90}` resolved where it expects `{70, 80, 90}`, reproducible
  three runs out of three. That entry proved it upstream's by reverting `crates/editor` wholesale
  to `upstream/main` and getting an identical failure, and the numbers tonight are identical again.
  The fork does not patch `code_lens.rs` and no upstream commit in these 32 touched it. Two weeks
  on, upstream still has not fixed it. The re-investigation this time cost a container's disk
  allowance before the 09-01 entry was found; the entry says in as many words that it exists so the
  chase is not repeated, which is a good argument for reading the log's failure notes before
  running the experiment rather than after.
- *`auto_update`'s `test_auto_update_downloads`* is new to this log, and it is upstream's too,
  proved the same way: with `crates/auto_update/src/auto_update.rs` reverted wholesale to
  `upstream/main` the test fails identically, panicking in `db.rs:88` with "database not
  initialized" by way of `set_should_show_update_notification`. The test never initialises a
  `db`, and on Linux the install path runs to completion and reaches the notification write.
  The suspicion worth recording, because it looked damning for about ten minutes: the fork widens
  a match arm to `ReleaseChannel::Nightly | ReleaseChannel::Dev` with no `debug_assertions` guard,
  which would be exactly the kind of change to drag a test down a new path. It is inert here —
  the test calls `release_channel::init_test(..., ReleaseChannel::Stable, ...)`, so both fork and
  upstream take the `_` arm. `-p auto_update` had never been in the gate set before tonight, which
  is why this surfaced now, same as `-p editor` on 09-01.
- *`gpui_macros`' doctests* fail with "attribute value must be a literal", which is the pair
  2026-09-01 already identified: the `#[gpui::property_test]` doc examples are written runnable,
  and a doctest of a proc-macro crate has no `gpui` in scope. Its unit and three integration tests
  pass. The fork does patch `property_test.rs` now — the `cx.quit()`-inside-`update` teardown fix —
  but it changes generated code only and not one `///` line, so the doc examples are upstream's
  verbatim and the 09-01 reading still holds.

**Environment, and two new prerequisites.** The standing two were needed as always
(`CARGO_NET_GIT_FETCH_WITH_CLI=true`, `libasound2-dev`). Two more turned up once the gate widened
past the core crates, both environment rather than code:

- `libx11-xcb-dev` (plus `libxcb1-dev`, `libxkbcommon-x11-dev`, `libxkbcommon-dev`,
  `libwayland-dev`). Test binaries for `agent_servers`, `git_ui` and `markdown` link X11 and failed
  with `ld: error: unable to find library -lX11-xcb`. The four core crates' binaries do not, which
  is why twenty-seven earlier nights of this log never hit it.
- `rsync`, for `auto_update`'s download path, which shells out to it on Linux and otherwise fails
  with its own "rsync is required for auto-updates" error before reaching the real failure above.

On the PPAs: `apt-get update` failed on `deadsnakes` and `ondrej` as it always does, but this run
had chained `update && install`, so the update's exit code stopped the install from ever running
and made an environment problem look worse than it was. Moving the two `.sources` files out of
`/etc/apt/sources.list.d/` and re-running both worked. The 09-15 advice still stands — only an
install failure is worth acting on — with the addition that the two should not be chained with
`&&`, or the advice cannot be followed.

**The disk allowance is the binding constraint on a wide gate, and it decides what fits.** It was
exhausted twice tonight, both times mid-link, which surfaces as `ld terminated with signal 7 [Bus
error]` rather than as anything mentioning space. `project` and `workspace` do not fit alongside
the rest of the debug tree; they need `rm -rf target` first and a run of their own. The ordering
that works is unchanged from 09-07 and 09-15: debug tests, then `rm -rf target/debug`, then the
release clippy, and never the two trees at once. Worth adding that `cargo clippy` only checks and
never codegens, so the release lint is ~5 minutes against the test build's hour — if time is short,
the lint is not what costs it.

**2026-09-17**: onto main 74646bf29 (24 upstream commits). Squash-then-rebase folded the standing
squash and the 09-16 rebase-log commit into one, reusing the squash's own message; tree-identical to
the old tip (`821e43640`) before rebasing. A working night by the more-than-one-commit trigger
alone, the same one as last night: both queues were empty going in, and the branch had moved
yesterday.

**Zero conflicts, and no markerless drift.** The rebase applied clean, and `cargo check --workspace
--all-targets` came back 0 errors and 0 warnings in 11 minutes. Third night running that the
conflict count was the whole story rather than the usual half of it.

**Eight of the fork's 90 changed files overlap with what upstream touched, every one auto-merged.**
Auto-merge is where a silent loss lives, so each was read rather than trusted.

- #64193 (ACP elicitation keyboard navigation) is the night's one real overlap. The fork does not
  patch `elicitation.rs`, but it does patch all three keymaps, and upstream added a nine-line
  `"Elicitation || (Elicitation > Editor)"` block to each. All three landed intact and all three
  still parse. Its four tests are the whole of `agent_ui` going 460 → 464, and they pass here. Worth
  knowing that none of the four is named `test_*` — they are `form_text_fields_submit_on_enter` and
  friends — so a grep for added `fn test_` reports that the commit added no tests, which is how a
  four-test gain nearly went down as unexplained.
- #64106 (title-bar feedback when already up to date) merged into `auto_update.rs`, the file
  carrying the fork's own Dev-channel release fetch, and it is an upstream feature the fork gets for
  free. The fork still constructs `UpdateVersion` in its title bar, and `quiet_ui_update::fetch_release`
  returns through the same no-newer-version branch that now emits `AutoUpdateEvent::UpToDate`, so a
  manual "Check for Updates" finding the fork's own release unchanged shows "Up to Date" for a few
  seconds instead of the checking indicator simply vanishing.
- #64184 added `test_cursor_animation_remains_active_during_keyboard_autoscroll` to
  `editor_tests.rs`, a fork-patched file; it is present alongside the fork's own
  `test_diff_review_button_auto_enabled_on_plain_editor`.
- #64340 (cargo shear) added `doctest = false` to `crates/sidebar/Cargo.toml`; #64292 and #64251
  moved the corgi patches under `tooling/` and added `objc2-screen-capture-kit` in the root
  `Cargo.toml`. All landed beside the fork's own three root-`Cargo.toml` additions (the `gh_status`
  member, its path entry, and `incremental = false`). The rest is `Cargo.lock` bumps.

**Nothing for the fork to delete.** The other twenty commits are gpui platform work (Windows shaders
and DirectWrite ligatures, the objc2 screen-capture migration, atlas and WebGL changes, macOS
traffic lights, centered window bounds), CI plumbing, and extension version bumps — none of it
builds anything this fork implements by hand. Two paths are worth naming because they read like fork
inventions and are not: `crates/agent_ui/src/ui/terminal_tool_header.rs` and
`crates/agent_ui/src/ui/sandbox_status_tooltip.rs` both exist upstream and this fork only *modifies*
them, so neither is ever a deletion candidate. Checked by name against upstream, along with the
four files that genuinely are fork-only (`quiet_ui_update.rs`, `generated_file.rs`,
`worktree_spares.rs`, `thread_read_state.rs`).

**The dead-`pub` sweep, run and empty again.** 58 plain `pub fn`s across the fork's thirteen own
modules, every one with a caller — the same 58 as 09-16, as expected since no fork code has landed
since. One note for whoever runs it next: counting `pub(crate)` as well gives 74, so a sweep
reporting a different number has most likely changed its pattern rather than found growth.

**The gate, seven crates: the core three plus the four where upstream landed in a fork-patched
file.** `cargo test`: `acp_thread` 241, `agent_ui` 464 (32 intentionally `#[ignore]`d), `sidebar`
180, `editor` 1141 (1 `#[ignore]`d), `auto_update` 16, `title_bar` 7, `ui` 82 (+41 doctests).
`script/clippy` (`--release --all-targets --all-features -- --deny warnings`) is clean across all
seven in one invocation, 5m21s. `cargo-shear`, `typos` and `buf` are not installed here, so
`script/clippy` exits after the lint, as it has every night in this log.

**One failure, already established as upstream's — and one that has gone away.**

- *`editor`'s `test_code_lens_resolve_only_visible`* failed again, with the assertion identical to
  the one 09-01 chased down and 09-16 re-confirmed: `{70, 90, 60, 80}` resolved where `{70, 80, 90}`
  is expected. Not re-investigated, on 09-16's own advice. Re-checked only the two cheap facts that
  would make it ours, and neither holds: the fork does not patch `code_lens.rs`, and none of
  tonight's 24 commits touches it either.
- *`auto_update`'s `test_auto_update_downloads`*, which 09-16 recorded as failing and proved
  upstream's (panicking in `db.rs:88` with "database not initialized"), passes tonight. The suite is
  16/16 green, up one test from 09-16's 15 because #64106 added one. #64106 is the only commit in
  this batch touching that file and it reworks precisely the branch the test runs through, so
  upstream looks to have fixed it in passing. Recorded so the next run does not go hunting a failure
  that is no longer there.

**Environment: the standing prerequisites, and 09-16's advice paying off.** The standing two were
needed as always (`CARGO_NET_GIT_FETCH_WITH_CLI=true`, `libasound2-dev`). `apt-get update` still
fails with 403s on the `deadsnakes` and `ondrej` PPAs, but following 09-16's note — do not chain it
to the install with `&&`, and act only on an install failure — the install ran on its own from the
cached lists and succeeded, so the `.sources` files did not need moving this time. 09-16's other two
prerequisites were installed up front rather than rediscovered (`libx11-xcb-dev` and its four
companions, `rsync`), and nothing failed to link.

**The disk allowance shaped the gate again, and cargo's fail-fast is half the story.** The debug
tree for the core three plus `editor` reached 18GB and the allowance ran out while building
`title_bar` and `ui` — as a plain `No space left on device` this time rather than 09-16's bus error,
which is much the friendlier of the two faces. But those two were only still outstanding because
`editor`'s one failure had aborted the earlier four-crate invocation before it reached them;
`--no-fail-fast` is what let the retry finish, and is worth passing whenever a known-failing crate
shares an invocation with others. The ordering that worked, extending the log's standing one:
debug tests for the core three, then for `editor`/`auto_update`, then `rm -rf target/debug`, then
the release clippy, then `rm -rf target/release` and a second short debug run for `title_bar` and
`ui`.

**Nothing was built: the Work queue was empty going in, and stays empty.** The Verification queue
was empty too. The night went to the squash, a clean rebase, the overlap survey above, and a
seven-crate gate.
