...
creates the worktree there and then, its cost is paid on every press, and it is already high enough
friction to be avoided. Seven seconds on a good run and thirty on a bad one, measured in three
phases the code still logs (`crates/git_ui_core/src/worktree_service.rs:1078`, `:1109`, `:1152`,
`:1365`). Take fresh numbers before optimising: the window-shown line was added since those, and the
picture may have moved.
...
message yet. Nothing about it should be special afterwards.

What goes with it: `DraftWorktreeChoice` and its three variants
(`crates/agent_ui/src/conversation_view/thread_view.rs:744`), the resolution that decides at send
time where a draft lands (`conversation_view.rs:1219`), the deferred creation path
`new_worktree_draft` plus `create_worktree_and_send`, and the persistence of the choice in
`draft_prompt_store`. `create_new_worktree_thread` already does the immediate version and says so in
its own doc comment ("Unlike a new-worktree draft, nothing is deferred"), so the direction is to
keep that one and let the rest go. Twenty-eight references across the crate say how much this has
been costing.

Two consequences, both accepted, both worth handling rather than discovering:

- `+` now waits for a worktree. Creation was measured at seven seconds on a good run and thirty on a
  bad one, and the window opening early helped but did not remove it. The thread has to be usable
  while that happens: typing must not be blocked, and a message typed before the worktree exists
  must land in it when it does.
- An abandoned `+` now leaves a real worktree on disk, where before it left nothing. The fork
  already records which worktrees it created so archival can remove them; make sure a thread
  abandoned without a message is covered by that, or this quietly fills the disk one click at a
  time.

One piece of the old logic is worth keeping, whatever shape it ends up in: a draft inside a linked
worktree sends *there* rather than making a second one, because that worktree was made for that
